...
export default class TickTickSync extends Plugin {
	...
	async onload() {
		...
		this.app.workspace.onLayoutReady(async () => {
			...
			//For some reason, we registered on create and called the pluginload on creation of every file.
			//don't know why we did that, but we're bloody stopping it.
	...
	private async pluginLoad() {
		...
		const ribbonIconEl1 = this.addRibbonIcon('check', 'TickTickSync', async (evt: MouseEvent) => {
			...
			if (file) {
				...
				// 	// })
				//
				await this.app.vault.modify(outFile, JSON.stringify(taskData, null, 4));
